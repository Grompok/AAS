    ...
    public function importAction() {
        ...
        if ($this->_request->isPost()) {
            if ($form->file->receive()) {
                ...
                $paramosAdministravimas = new Application_Model_DbTable_ParamosAdministravimas();
                
                $cols = $data->sheets[4]['numRows'];
                for ($i = 2; $i < $cols; $i++) {
                    for ($j = 2; $j < $data->sheets[4]['numCols']; $j++) {
                        if (isset($data->sheets[4]['cells'][$i][$j])
                                && (is_integer($data->sheets[4]['cells'][$i][$j])
                                || is_float($data->sheets[4]['cells'][$i][$j]))) {
                            //$data->sheets[4]['cells'][$i][$j]; //reiksme
                            //$data->sheets[4]['cells'][$i][1]; //priemones kodas
                            // $data->sheets[4]['cells'][1][$j]; //padalino kodas
                            try {
                                $paramosAdministravimas->insert(array(
                                    'Priem_kodas' => strtolower($data->sheets[4]['cells'][$i][1]),
                                    'Pad_kodas' => $data->sheets[4]['cells'][1][$j],
                                    'Valandos' => $data->sheets[4]['cells'][$i][$j],
                                ));
                            } catch (Zend_Db_Exception $zbe) {
                                //echo $zbe->getMessage();
                            }
